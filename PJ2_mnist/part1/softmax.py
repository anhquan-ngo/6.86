...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    one_hot = np.zeros((k, n),dtype=bool)
    one_hot[Y,np.arange(len(Y))] = True

    # Compute scores and probabilities for each class
    probs = compute_probabilities(X,theta,temp_parameter)

    # Compute the loss and regularization terms
    loss = -np.sum(np.log(probs)[one_hot]) / n
    reg = 0.5 * lambda_factor * np.sum(theta**2)
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    derivatite = -1/(temp_parameter*n)*((M-H)@X) + lambda_factor*theta
    theta -= alpha*derivatite
    return theta
